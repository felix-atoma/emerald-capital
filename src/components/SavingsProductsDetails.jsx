            ...
            <div className="bg-gradient-to-r from-yellow-50 to-purple-50 rounded-2xl p-8 shadow-lg border border-purple-100">
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-xl font-bold text-gray-900">
                  {calculatorType === 'savings' ? 'Savings Calculator' : 'Loan Calculator'}
                </h3>
                <button
              ...
              <div className="space-y-6">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    {calculatorType === 'savings' ? 'Monthly Savings' : 'Loan Amount'}
                  </label>
                  <div className="flex items-center border-2 border-purple-200 rounded-xl overflow-hidden">
                    ...
                    <input
                      type="range"
                      min={calculatorType === 'savings' ? 50 : 1000}
                      max={calculatorType === 'savings' ? 10000 : 100000}
                      step={calculatorType === 'savings' ? 50 : 1000}
                      value={calculatorType === 'savings' ? monthlySavings : loanAmount}
                      onChange={(e) => calculatorType === 'savings' 
                        ? setMonthlySavings(Number(e.target.value))
                        : setLoanAmount(Number(e.target.value))
                      }
                      className="flex-1 px-4 py-3"
                    />
                    <div className="bg-purple-50 px-4 py-3 border-l">
                      <input
                        type="number"
                        value={calculatorType === 'savings' ? monthlySavings : loanAmount}
                        onChange={(e) => calculatorType === 'savings'
                        ...
                        className="w-32 text-right outline-none bg-transparent font-semibold"
                      />
                    </div>
                  </div>
                  <div className="flex justify-between text-sm text-gray-500 mt-2">
                    <span>Min: ₵{calculatorType === 'savings' ? '50' : '1,000'}</span>
                    <span>Max: ₵{calculatorType === 'savings' ? '10,000' : '100,000'}</span>
                  </div>
                </div>
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    {calculatorType === 'savings' ? 'Savings Period' : 'Loan Term'}
                  </label>
                  <div className="flex items-center gap-4">
                    <input
                      type="range"
                      min={calculatorType === 'savings' ? 1 : 6}
                      max={calculatorType === 'savings' ? 60 : 84}
                      value={calculatorType === 'savings' ? savingsPeriod : loanPeriod}
                      onChange={(e) => calculatorType === 'savings'
                        ? setSavingsPeriod(Number(e.target.value))
                    ...
                    <select
                      ...
                      className="w-32 border-2 border-purple-200 rounded-xl px-4 py-3 outline-none"
                    >
                      {calculatorType === 'savings' ? (
                        <>
                ...
                <div>
                  ...
                  <div className="flex items-center gap-4">
                    <input
                      ...
                      min="1"
                      max="25"
                      value={calculatorType === 'savings' ? interestRate : loanInterestRate}
                      onChange={(e) => calculatorType === 'savings'
                      ...
                      className="flex-1"
                    />
                    <div className="flex items-center border-2 border-purple-200 rounded-xl px-4 py-3 w-32">
                      <input
                        type="number"
                        min="1"
                        max="25"
                        ...
                        className="w-full text-right outline-none font-semibold"
                      />
                      <Percent className="w-4 h-4 ml-2 text-gray-400" />
                    </div>
                  </div>
                </div>

                {/* Results */}
                <div className="bg-white rounded-xl p-6 border border-purple-200 space-y-4">
                  {calculatorType === 'savings' ? (
                    <>
                      <div className="flex justify-between items-center">
                        <div>
                          <span className="text-gray-600">Total Savings</span>
                          <p className="text-sm text-gray-500">₵{monthlySavings} × {savingsPeriod} months</p>
                        </div>
                        <span className="font-bold text-gray-900 text-lg">₵{totalSavings.toLocaleString()}</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <div>
                          <span className="text-gray-600">Interest Earned</span>
                          <p className="text-sm text-gray-500">@{interestRate}% per annum</p>
                        </div>
                        <span className="font-bold text-green-600 text-lg">+₵{interestEarned.toLocaleString()}</span>
                      </div>
                      <div className="flex justify-between items-center pt-4 border-t">
                        <span className="text-gray-600 font-semibold text-xl">Final Amount</span>
                        <span className="font-bold text-2xl text-purple-600">₵{finalAmount.toLocaleString()}</span>
                      </div>
                    </>
                  ) : (
                    <>
                      <div className="flex justify-between items-center">
                        <div>
                          ...
                          <p className="text-sm text-gray-500">{loanPeriod} payments</p>
                        </div>
                        <span className="font-bold text-gray-900 text-lg">₵{monthlyPayment.toLocaleString()}</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <div>
                          <span className="text-gray-600">Total Interest</span>
                          <p className="text-sm text-gray-500">@{loanInterestRate}% APR</p>
                        </div>
                        <span className="font-bold text-red-600 text-lg">₵{totalInterestPaid.toLocaleString()}</span>
                      </div>
                      <div className="flex justify-between items-center pt-4 border-t">
                        <span className="text-gray-600 font-semibold text-xl">Total Loan Cost</span>
                        <span className="font-bold text-2xl text-purple-600">₵{totalLoanCost.toLocaleString()}</span>
                      </div>
                    </>
                  )}
                </div>

                {/* Savings Frequency (only for savings) */}
                {calculatorType === 'savings' && (
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">Savings Frequency</label>
                    <div className="grid grid-cols-3 gap-2">
                      {['Monthly', 'Quarterly', 'Yearly'].map((freq) => (
                        <button
                          key={freq}
                          onClick={() => setSavingsFrequency(freq.toLowerCase())}
                          className={`py-3 rounded-lg font-medium ${
                            savingsFrequency === freq.toLowerCase()
